...
<div class="min-h-screen py-12 relative overflow-hidden bg-gradient-to-b from-space-950 to-cosmic-900">
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto bg-black/30 backdrop-blur-xl rounded-2xl overflow-hidden border border-white/10 relative transition-all duration-300 hover:border-white/20 hover:shadow-[0_0_30px_rgba(147,51,234,0.3)]">
            <!-- Navegación -->
            <div class="p-6">
                <a href="{{ route('blog') }}" class="text-white/90 hover:text-white flex items-center gap-2 bg-white/5 px-4 py-2 rounded-lg w-fit transition-all duration-300 hover:bg-white/10">
            ...
            <div class="relative overflow-hidden mb-6">
                @if($noticia->imagen)
                    ...
                    <div class="relative z-20 px-8 py-4 bg-black/30 backdrop-blur-sm border-t border-white/10">
                        ...
                        <p class="text-white/90 text-sm mb-1 text-justify"><span class="font-medium">Descripción:</span> {{ $noticia->descripcion_imagen }}</p>
                        @endif
                        @if($noticia->autor_imagen)
                        <p class="text-white/90 text-sm"><span class="font-medium">Créditos:</span> {{ $noticia->autor_imagen }}</p>
                        @endif
                    </div>
                    @endif
                @endif
                ...
                <div class="relative z-20 p-8">
                    <h1 class="text-5xl font-bold text-white mb-4 text-center drop-shadow-lg">{{ $noticia->titulo }}</h1>
