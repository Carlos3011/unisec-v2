...
@section('titulo', 'Editar Concurso')

@section('contenido')
<div class="bg-gradient-to-br from-gray-900 via-gray-800 to-black rounded-2xl shadow-2xl p-8 max-w-4xl mx-auto">
    <div class="mb-8">
        <div class="flex justify-between items-center">
            <h1 class="text-3xl font-bold bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 bg-clip-text text-transparent tracking-tight">Editar Concurso</h1>
    ...
    <form action="{{ route('admin.concursos.update', $concurso) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $concurso->titulo) }}" required
                ...
                <div class="space-y-2">
                    ...
                    <select name="tema_id" id="tema_id" required
                        ...
                        @foreach($temas as $tema)
                            <option value="{{ $tema->id }}" {{ old('tema_id', $concurso->tema_id) == $tema->id ? 'selected' : '' }}>
                                {{ $tema->nombre }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="estado" id="estado" class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-white">
                        <option value="pendiente" {{ old('estado', $concurso->estado) == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                        <option value="activo" {{ old('estado', $concurso->estado) == 'activo' ? 'selected' : '' }}>Activo</option>
                        <option value="inactivo" {{ old('estado', $concurso->estado) == 'inactivo' ? 'selected' : '' }}>Inactivo</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end space-x-4 mt-8">
            <a href="{{ route('admin.concursos.index') }}" class="px-6 py-3 bg-gray-600/80 text-white rounded-xl hover:bg-gray-700 transform hover:scale-105 transition-all duration-200">
                Cancelar
            </a>
