...
<div class="relative z-10">
    <div class="bg-gradient-to-br from-gray-900 to-black rounded-xl shadow-xl p-8 mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="relative">
                <input type="text" placeholder="Buscar concursos..." class="w-full bg-gray-800 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
            ...
            <select class="bg-gray-800 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
                ...
                <option value="3">Robótica Espacial</option>
            </select>
            <select class="bg-gray-800 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-white">
                <thead class="bg-gray-800 text-gray-300">
                    <tr>
                        <th class="px-4 py-3 text-left">Nombre del Concurso</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-800/50">
                        <td class="px-4 py-3">{{ $concurso->nombre }}</td>
                        <td class="px-4 py-3">{{ $concurso->categoria }}</td>
                        <td class="px-4 py-3 text-center">{{ $concurso->fecha_limite }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="px-3 py-1 rounded-full text-xs {{ $concurso->estado == 'activo' ? 'bg-green-500/20 text-green-500' : 'bg-gray-500/20 text-gray-500' }}">
                                {{ ucfirst($concurso->estado) }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-center">{{ $concurso->participantes_count }}</td>
                        <td class="px-4 py-3 text-center">
                            <div class="flex justify-center space-x-2">
                                <a href="{{ route('admin.concursos.edit', $concurso) }}" class="text-blue-400 hover:text-blue-300">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('admin.concursos.destroy', $concurso) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
