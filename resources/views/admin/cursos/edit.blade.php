        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <textarea name="descripcion" id="descripcion" rows="4" required
                    class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('descripcion', $curso->descripcion) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="ponente_id" id="ponente_id" required
                    ...
                    @foreach($ponentes as $ponente)
                        <option value="{{ $ponente->id }}" {{ old('ponente_id', $curso->ponente_id) == $ponente->id ? 'selected' : '' }}>
                            {{ $ponente->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="costo" id="costo" value="{{ old('costo', $curso->costo) }}" step="0.01" required
                    class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>

            <div>
                <label for="fecha_inicio" class="block text-sm font-medium mb-2">Fecha de Inicio</label>
                <input type="date" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $curso->fecha_inicio) }}" required
                    class="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
