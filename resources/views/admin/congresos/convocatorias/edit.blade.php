...
<div class="bg-gray-800 bg-opacity-50 p-6 rounded-lg shadow-lg">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-bold text-white flex items-center">
            <i class="fa-solid fa-graduation-cap mr-3 text-blue-400"></i>Editar Convocatoria de Congreso
        </h1>
    ...
    <form action="{{ route('admin.congresos.convocatorias.update', $convocatoria) }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="{
        currentTab: 'general',
        progress: 20,
        tabs: {
            general: true,
        ...
        fechasImportantes: {{ Js::from($convocatoria->fechasImportantes->map(fn($fecha) => [
            'titulo' => $fecha->titulo,
            'fecha' => $fecha->fecha
        ])->toArray()) }},
        addFecha() {
        ...
        tematicas: {{ Js::from($convocatoria->tematicas) }},
        addTematica() {
        ...
        <div x-show="currentTab === 'general'" class="space-y-6 animate-fade-in">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="congreso_id" id="congreso_id" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500" required>
                        ...
                        @foreach($congresos as $congreso)
                            <option value="{{ $congreso->id }}" {{ $convocatoria->congreso_id == $congreso->id ? 'selected' : '' }}>{{ $congreso->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="sede" id="sede" value="{{ $convocatoria->sede }}" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500" required>
                </div>

                <div>
                    <label for="dirigido_a" class="block text-sm font-medium text-gray-300">Dirigido a</label>
                    <input type="text" name="dirigido_a" id="dirigido_a" value="{{ $convocatoria->dirigido_a }}" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500" required>
                </div>

                <div>
                    <label for="contacto_email" class="block text-sm font-medium text-gray-300">Email de Contacto</label>
                    <input type="email" name="contacto_email" id="contacto_email" value="{{ $convocatoria->contacto_email }}" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500">
                </div>
            </div>
        </div>
        ...
        <div x-show="currentTab === 'requisitos'" class="space-y-6 animate-fade-in">
            <div>
                ...
                <textarea name="requisitos" id="requisitos" rows="6" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500" required>{{ $convocatoria->requisitos }}</textarea>
            </div>
                    ...
                    <template x-for="(tematica, index) in tematicas" :key="index">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-700/50 rounded-lg">
                            <div>
                                ...
                                <input type="text" :name="'tematicas[' + index + '][titulo]'" :id="'tematica_titulo_' + index" x-model="tematica.titulo" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500" required>
                            </div>
                            <div>
                                <label :for="'tematica_descripcion_' + index" class="block text-sm font-medium text-gray-300">Descripción</label>
                                <input type="text" :name="'tematicas[' + index + '][descripcion]'" :id="'tematica_descripcion_' + index" x-model="tematica.descripcion" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500" required>
                            </div>
                            <div class="md:col-span-2 flex justify-end">
        ...
        <div x-show="currentTab === 'documentos'" class="space-y-6 animate-fade-in">
            <div>
                ...
                <textarea name="formato_articulo" id="formato_articulo" rows="4" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500" required>{{ $convocatoria->formato_articulo }}</textarea>
            </div>

            <div>
                <label for="formato_extenso" class="block text-sm font-medium text-gray-300">Formato de Extenso</label>
                <textarea name="formato_extenso" id="formato_extenso" rows="4" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500" required>{{ $convocatoria->formato_extenso }}</textarea>
            </div>
        </div>
        ...
        <div x-show="currentTab === 'evaluacion'" class="space-y-6 animate-fade-in">
            <div>
                ...
                <textarea name="criterios_evaluacion" id="criterios_evaluacion" rows="6" class="mt-1 block w-full rounded-md bg-gray-700 border-gray-600 text-white focus:border-blue-500 focus:ring-blue-500" required>{{ $convocatoria->criterios_evaluacion }}</textarea>
            </div>
        </div>

        <!-- Pestaña Pagos -->
        <div x-show="currentTab === 'pagos'" class="space-y-6 animate-fade-in">
            <div x-data="{ cuotas: {{ Js::from($convocatoria->cuotas_inscripcion ?: [['rol' => '', 'monto' => '']]) }} }" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    @if($convocatoria->archivo_convocatoria)
                        <div class="mb-2 text-sm text-gray-400">Archivo actual: <a href="{{ asset($convocatoria->archivo_convocatoria) }}" target="_blank" class="text-blue-400 hover:text-blue-300">Ver archivo</a></div>
                    @endif
                ...
                <div>
                    ...
                    @if($convocatoria->archivo_articulo)
                        <div class="mb-2 text-sm text-gray-400">Archivo actual: <a href="{{ asset($convocatoria->archivo_articulo) }}" target="_blank" class="text-blue-400 hover:text-blue-300">Ver archivo</a></div>
                    @endif
                ...
                <div class="md:col-span-2">
                    ...
                    @if($convocatoria->imagen_portada)
                        <div class="mb-2">
                            <img src="{{ asset($convocatoria->imagen_portada) }}" alt="Imagen de portada actual" class="h-32 object-cover rounded-lg">
                        </div>
                    @endif
